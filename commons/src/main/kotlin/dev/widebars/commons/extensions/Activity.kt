...
import dev.widebars.commons.databinding.DialogTitleBinding
import dev.widebars.commons.dialogs.*
import dev.widebars.commons.dialogs.WritePermissionDialog.WritePermissionDialogMode
import dev.widebars.commons.helpers.*
import dev.widebars.commons.helpers.MyContentProvider.COL_LAST_UPDATED_TS
import dev.widebars.commons.helpers.MyContentProvider.MY_CONTENT_URI
import dev.widebars.commons.models.*
import dev.widebars.commons.views.MyTextView
import java.io.*
...
fun Activity.openPathIntent(
    ...
    ensureBackgroundThread {
        ...
        Intent().apply {
            ...
            if (applicationId == "dev.widebars.gallery" || applicationId == "dev.widebars.gallery.debug" ||
                applicationId == "dev.widebars.gallery" || applicationId == "dev.widebars.gallery.debug") {
    ...
    handlePermission(PERMISSION_CALL_PHONE) {
        ...
        Intent(action).apply {
            ...
            if (isDefaultDialer()) {
                ...
                val packageName = if (baseConfig.appId.contains(".debug", true)) prefix + "widebars.dialer.debug" else prefix + "widebars.dialer"
                val className = prefix + "widebars.dialer.activities.DialerActivity"
