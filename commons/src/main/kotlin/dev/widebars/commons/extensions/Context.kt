...
import dev.widebars.commons.helpers.MyContentProvider.PERMISSION_WRITE_GLOBAL_SETTINGS
import dev.widebars.commons.models.AlarmSound
import dev.widebars.commons.models.BlockedNumber
import dev.widebars.commons.models.contacts.ContactRelation
...
fun Context.getFilePublicUri(file: File, applicationId: String): Uri {
    ...
    // if media content uri is null, get our custom uri like content://dev.widebars.gallery.provider/external_files/emulated/0/DCIM/IMG_20171104_233915.jpg
...
fun Context.isOrWasThankYouInstalled(allowPretend: Boolean = true): Boolean {
    return when {
        isPackageInstalled("dev.widebars.audiobook")
            || isPackageInstalled("dev.widebars.voicerecorder")
            || isPackageInstalled("dev.widebars.files") -> {
...
fun Context.getCanAppBeUpgraded() = proPackages.contains(baseConfig.appId.removeSuffix(".debug").removePrefix("dev.widebars."))
...
fun Context.isDefaultDialer(): Boolean {
    return if (!packageName.startsWith("dev.widebars.contacts") && !packageName.startsWith("dev.widebars.dialer") &&
        !packageName.startsWith("dev.widebars.contacts") && !packageName.startsWith("dev.widebars.dialer")) {
        true
    } else if ((packageName.startsWith("dev.widebars.contacts") || packageName.startsWith("dev.widebars.dialer") ||
            packageName.startsWith("dev.widebars.contacts") || packageName.startsWith("dev.widebars.dialer")) && isQPlus()) {
...
fun Context.isCollection(): Boolean {
    ...
    return isPackageInstalled(prefix + "widebars.dialer")
        && isPackageInstalled(prefix + "widebars.contacts")
        && isPackageInstalled(prefix + "widebars.smsmessenger")
        && isPackageInstalled(prefix + "widebars.gallery")
        && isPackageInstalled(prefix + "widebars.audiobooklite")
        && isPackageInstalled(prefix + "widebars.filemanager")
        && isPackageInstalled(prefix + "widebars.keyboard")
        && isPackageInstalled(prefix + "widebars.calendar")
        && isPackageInstalled(prefix + "widebars.voicerecorderfree")
}

fun Context.appPrefix(): String = if (packageName.startsWith("dev.widebars")) "dev." else "com."
...
fun Context.googlePlayDevUrlRes(): Int {
    return when {
        packageName.startsWith("dev.widebars") -> R.string.google_play_dev_url
...
fun Context.myMailRes(): Int {
    return when {
        packageName.startsWith("dev.widebars") -> R.string.my_email
        packageName.startsWith("dev.widebars") -> R.string.my_email_old
