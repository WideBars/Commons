...
import dev.widebars.commons.compose.screens.ManageBlockedNumbersScreen
import dev.widebars.commons.compose.theme.AppThemeSurface
import dev.widebars.commons.dialogs.AddOrEditBlockedNumberAlertDialog
import dev.widebars.commons.dialogs.ExportBlockedNumbersDialog
import dev.widebars.commons.dialogs.RadioGroupAlertDialog
...
class ManageBlockedNumbersActivity : BaseSimpleActivity() {
    ...
    override fun onCreate(savedInstanceState: Bundle?) {
        ...
        setContent {
            ...
            val isDialer = remember {
                config.appId.startsWith(prefix + "widebars.dialer")
            }
            ...
            AppThemeSurface {
                ...
                blockingTypeDialogState.DialogMember {
                    RadioGroupAlertDialog(
                        ...
                        items = if (isQPlus()) {
                            listOf(
                                RadioItem(BLOCKING_TYPE_REJECT, stringResource(id = dev.widebars.strings.R.string.blocking_type_reject)),
                                RadioItem(BLOCKING_TYPE_DO_NOT_REJECT, stringResource(id = dev.widebars.strings.R.string.blocking_type_do_not_reject)),
                                RadioItem(BLOCKING_TYPE_SILENCE, stringResource(id = dev.widebars.strings.R.string.blocking_type_silence)),
                            ).toImmutableList()
                        } else {
                            listOf(
                                RadioItem(BLOCKING_TYPE_REJECT, stringResource(id = dev.widebars.strings.R.string.blocking_type_reject)),
                        ...
                        titleId = dev.widebars.strings.R.string.blocking_type,
                        cancelCallback = {}
                    ) { item ->
    ...
    private fun maybeSetDefaultCallerIdApp() {
        val prefix = appPrefix()
        if (isQPlus() && baseConfig.appId.startsWith(prefix + "widebars.dialer")) {
